...
function switchScene(scene) {
  ...
  // สร้างเมนู "จุดข้อมูล" จาก infoHotspots ใน data.js อัตโนมัติ
  function buildInfoNavFromInfoHotspots() {
    var navItems = document.querySelector('#audioNav .audio-nav-items');
    if (!navItems || !data || !data.scenes) return;
    ...
    header.className = 'audio-nav-header';
    header.textContent = 'จุดข้อมูล / องค์พระ';
    navItems.appendChild(header);

    data.scenes.forEach(function(sceneData) {
      if (!sceneData.infoHotspots || sceneData.infoHotspots.length === 0) return;

      sceneData.infoHotspots.forEach(function(hotspot) {
        var item = document.createElement('div');
        item.className = 'audio-nav-item info-nav-item';

        item.innerHTML =
          '<div class="item-icon">ℹ️</div>' +
          '<div class="item-info">' +
            '<div class="item-name">' + sanitize(hotspot.title || 'ข้อมูล') + '</div>' +
            '<div class="item-sub">' + sanitize(sceneData.name || 'จุดข้อมูล') + '</div>' +
          '</div>' +
          '<div class="item-arrow">→</div>';

        item.addEventListener('click', function() {
          var scene = findSceneById(sceneData.id);
          if (!scene) return;

          switchScene(scene);

          // หันกล้องไปยังตำแหน่ง infoHotspot ที่เลือก
          setTimeout(function() {
